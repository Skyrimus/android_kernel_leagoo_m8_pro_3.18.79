...
static VOID update_consys_rom_desel(VOID)
{
	...
	WMT_PLAT_INFO_FUNC("Update consys rom desel value(0x%x)\n",
...
static VOID consys_hang_debug_info(VOID)
{
	...
	WMT_PLAT_INFO_FUNC("addr1:%zx:0x%x/addr2:%zx/addr3:%zx:0x%x\n",
	...
	WMT_PLAT_INFO_FUNC("0x%x->addr1, addr2:0x%x/0x%x->addr1, addr2:0x%x\n",
	...
	for (i = 0; i < 9; i++) {
		...
		CONSYS_REG_WRITE(addr_1, wv1[1]);
		rv2[1] = CONSYS_REG_READ(addr_2);
		WMT_PLAT_INFO_FUNC("0x%x->addr3,0x%x->addr1,addr2:0x%x/0x%x->addr1,addr2:0x%x\n",
				   wv3, wv1[0], rv2[0], wv1[1], rv2[1]);
	}

	WMT_PLAT_INFO_FUNC("apb0_dbg_prob\n");
	for (i = 0; i < 10; i++) {
	...
	WMT_PLAT_INFO_FUNC("apb1_dbg_prob\n");
	for (i = 0; i < 10; i++) {
...
static INT32 consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	if (enable) {
		/*set vcn28 SW mode*/
		KERNEL_upmu_set_reg_value(MT6358_LDO_VCN28_OP_EN, 0x1);
